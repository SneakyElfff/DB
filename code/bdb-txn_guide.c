...
int main(int argc, char *argv[]) {
    ...
    db_home_dir = "./";
#endif
    while ((ch = getopt(argc, argv, "h:")) != EOF) switch (ch) {
        case 'h':
            db_home_dir = optarg;
            break;
        case '?':
...
void* writer_thread(void *args) {
    ...
    for (i = 0; i < 50; i++) {
        ...
        // Some think it is bad form to loop with a goto statement, but we do it anyway because it is the 
        // simplest and clearest way to achieve our abort/retry operation.
    retry:
        //    Begin our transaction. We group multiple writes in this thread under a single transaction 
        // so as to (1) show that you can atomically perform multiple writes at a time, and (2) to 
        // increase the chances of a deadlock occurring so that we can observe our deadlock detection at work.
        //    Normally we would want to avoid the potential for deadlocks, so for this workload the correct 
        // thing would be to perform our puts with autocommit. But that would excessively simplify our example, 
        // so we do the "wrong" thing here instead.
        ...
        for (j = 0; j < 10; j++) {
            ...
            case DB_KEYEXIST:
                ...
                // It must abort its operation, and optionally retry the put.
